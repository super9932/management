...
import InsuranceCommonTable from '../components/InsuranceCommonTable';
import InsuranceCommonDetailDialog from '../components/InsuranceCommonDetailDialog';
...
function InsuranceCommonViewInner() {
  const {
  ...
  } = useManualDocuments(MANUAL_ADMIN_TYPE.insuranceCommon);
  const [detailRow, setDetailRow] = useState<InsuranceCommonRow | null>(null);
    ...
    <>
      ...
      <Box sx={{ mb: 5, pt: 3 }}>
        <Breadcrumbs separator={<NavigateNextIcon sx={{ fontSize: 14 }} />} sx={{ mb: 1 }}>
          ...
          <Typography sx={{ fontSize: 14, color: DISABLED }}>보험공통 문서</Typography>
        </Breadcrumbs>
      ...
      <DocumentRegisterDialog
        open={registerOpen}
        adminType={MANUAL_ADMIN_TYPE.insuranceCommon}
        title="보험공통 문서 등록"
...
export default function InsuranceCommonView() {
  return (
    <NabThemeScope>
      <InsuranceCommonViewInner />
    </NabThemeScope>
  );
}
