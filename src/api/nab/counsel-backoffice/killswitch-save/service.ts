...
export const saveCounselKillSwitch = async (data: CounselKillSwitchSaveRequest) => {
  const url = NAB_COUNSEL_BACKOFFICE_API.killSwitchSave;
  const response = await axiosInstance.post<ApiResponse<CounselKillSwitchSaveResponse>>(url, data);
