 ...
 */
export const deleteStipulation = async (data: StipulationDeleteRequest, emnb: string) => {
  const url = NAB_COUNSEL_BACKOFFICE_API.stipulationDelete;
  const response = await axiosInstance.post<ApiResponse<StipulationDeleteResponse>>(url, withEmnb(data, emnb));
