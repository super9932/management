...
export const initLoadStipulations = async (data: StipulationInitLoadRequest) => {
  const url = NAB_COUNSEL_BACKOFFICE_API.stipulationInitLoad;
  const response = await axiosInstance.post<ApiResponse<StipulationInitLoadResponse>>(url, data);
