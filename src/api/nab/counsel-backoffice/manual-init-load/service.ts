...
export const initLoadManuals = async (data: ManualInitLoadRequest, emnb: string) => {
  const url = NAB_COUNSEL_BACKOFFICE_API.manualInitLoad;
  const response = await axiosInstance.post<ApiResponse<ManualInitLoadResponse>>(url, withEmnb(data, emnb));
